...
export class TransactionsService {
  ...
  async creditWallet(userId: number, createTx: CreateTransaction) {
    ...
    const wallet = await this.walletRepo.findOne({
      where: { user: userId, currency: createTx.currency },
    });
  ...
  async verifyTransaction(txId: number) {
    ...
    if (verify) {
      ...
      await this.walletRepo.update(
        { id: tx.wallet },
        { balance: wallet.balance + tx.txAmount },
      );
      return verify;
    } else {
  ...
  async getMonthlyTx(userId: number, monthTx: MonthlyTx) {
    ...
    if (user.role === 'admin') {
      ...
      return await this.transactionRepo.find({
        where: { txdate: Between(startDate, endDate) },
      });
    } else {
  ...
  async transfer(userId: number, transfer: TransferWallet) {
    ...
    const userWallet = await this.walletRepo.findOne({
      where: { user: userId, currency: transfer.currency },
    });
    const recipient = await this.userRepo.findOneBy({
      id: transfer.reciepient,
    });
    const reciepientWallet = await this.walletRepo.findOne({
      where: { user: transfer.reciepient, currency: transfer.currency },
    });
    if (userWallet.balance > transfer.amount) {
      if (transfer.amount < 1000000) {
        await this.walletRepo.update(userWallet.id, {
          balance: userWallet.balance - transfer.amount,
        });
        await this.walletRepo.update(reciepientWallet.id, {
          balance: reciepientWallet.balance + transfer.amount,
        });
        const newTx = this.transactionRepo.create({
  ...
  async approveTransfer(txId: number, userId: number) {
    ...
    if (user.role === 'admin') {
      ...
      await this.walletRepo.update(reciepientWallet.id, {
        balance: reciepientWallet.balance + tx.txAmount,
      });
    } else {
